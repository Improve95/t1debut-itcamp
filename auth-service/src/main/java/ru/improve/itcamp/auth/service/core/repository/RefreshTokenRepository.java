...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, String> {
    ...
    @Modifying
    @Query(nativeQuery = true, value = "insert into white_list_refresh_tokens(token, user_id) values (:token, :userId)")
    ...
    @Query(
            ...
            value = """
                select count(wt.token) > 0 
                    from (select wl.token from white_list_refresh_tokens wl where wl.user_id = :userId) as wt 
                    where wt.token = :token
                """)
    boolean existsAccessTokenByTokenAndUserId(String token, int userId);

    @Modifying
    @Query(nativeQuery = true, value = "delete from white_list_refresh_tokens wl where wl.user_id = :userId")
